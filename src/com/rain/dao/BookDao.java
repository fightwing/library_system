...
public class BookDao {
	 ...
	 * @param num
	 */
	public void addBook(String card, String name, String type, String autho, String press, int num, int nownum) {
		// TODO Auto-generated method stub
		Connection conn = DBUtil.getConnectDb();
		String sql = "insert  into book(card,name,type,autho,press,num,nownum) values(?,?,?,?,?,?,?)";
		int rs = 0;
	...
	public ArrayList<BookBean> get_ListInfo(){
		...
		try {
			...
			while(rs.next()){
				...
				tag.setNownum(rs.getInt("nownum"));
				tag_Array.add(tag);
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	/**
	 * 获取单个图书的信息，根据传入的hid来查找，返回一个HistoryBean数据类型
	 * @param hid
	 * @return
	 */
	 ...
	 * 修改图书的信息，bid作为条件，
	 */
	public void updateBook(int bid, String card, String name, String type, String autho, String press, int num, int nownum) {
		// TODO Auto-generated method stub
		Connection conn = DBUtil.getConnectDb();
		String sql = "update book set name=?,card=?,type=?,autho=?,press=?,num=?,nownum=? where bid=?";
		PreparedStatement stm = null;
		try {
			stm = conn.prepareStatement(sql);
	...
	public static String plusDay(int num){
        ...
        Calendar c = Calendar.getInstance();
        c.add(Calendar.DATE, num);// num为增加的天数，可以改变的
        d = c.getTime();
        String endtime = format.format(d);
        
        return endtime;
    }
	public void renewbook(int hid) {
		// TODO Auto-generated method stub
		//生成日期
		
		String endtime= plusDay(30);
	...
	public BookBean get_CardInfo(String card){
		BookBean tag = new BookBean();
		Connection conn = DBUtil.getConnectDb();
		String sql = "select * from book where card='"+card+"'";
		PreparedStatement stm = null;
		ResultSet rs = null;
		try {
		stm = conn.prepareStatement(sql);
